...
3. 选择模型提供商：
   ...
   - **Custom**: 自定义 API 端点（支持兼容 OpenAI 协议的服务）
4. 输入 API Key 和基础 URL
5. 点击 **保存**
...
| **Epic** | 大的功能模块/里程碑 |
| **Story** | 可交付的用户功能 |
| **Skill** | AI 技能配置，用于扩展 AI 能力 |
| **Agent** | AI 执行者角色，执行复杂任务 |
...
| 变量 | 默认值 | 描述 |
|------|--------|------|
| `PORT` | 3333 | 后端服务端口（同时提供 API 和前端静态文件） |
| `FRONTEND_PORT` | 5173 | 前端开发服务器端口（开发模式） |
| `WORKSPACE_PATH` | /workspace | 工作区目录 |
| `TLS_ENABLED` | false | 是否启用 TLS 1.3 加密传输 |
...
```bash
# 生成自签名证书
deno run --allow-all scripts/generate-cert.ts

# 启动服务器
TLS_ENABLED=true deno run --allow-all backend/src/main.ts
```

**注意：**
- Docker 部署时，容器会自动生成自签名证书
- 自签名证书在浏览器中会显示安全警告，这是正常现象
- 生产环境建议使用正式 CA 签发的证书
...
- [ ] 8.1 健康检查端点正常：`curl http://localhost:3333/health`
- [ ] 8.2 API 响应正常
